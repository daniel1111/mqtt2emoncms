...
$base_path = __DIR__ . "/";
$config_file =  $base_path . "../mqtt2emoncms.conf";
...
if (isset($argv[1]))
{
  ...
  else if ($filename == "INSTALL")
  {
    // (re)load all SPs, and load all tables that don't currently exists
    ...
    foreach($aFileList as $file) 
    {
      if (is_table($file))
      {
        $table_name = str_ireplace("tb_" , "", $file);
  ...
  else if (is_table($filename))
  {
    ...
    if (file_exists($filename))    
      ...
      load_file($base_path . "schema/" . $filename, $aConfig, $oDB);      
  } 
